...
(defrule verdura-suma-saludable
  ...
  (test (or (member$ lechuga $?ingredientes)
            (member$ tomate $?ingredientes)
            (member$ cebolla $?ingredientes)
            (member$ espinaca $?ingredientes)
            (member$ calabacin $?ingredientes)
            (member$ berenjena $?ingredientes)
            (member$ brocoli $?ingredientes)
            (member$ pimientos_rojos $?ingredientes)
            (member$ cebolla_de_verdeo $?ingredientes)
            (member$ zanahoria $?ingredientes)
            (member$ col $?ingredientes)
...
(deffunction combinacion (?cf1 ?cf2)
  (if (and (> ?cf1 0) (> ?cf2 0))
    then (bind ?rv (- (+ ?cf1 ?cf2) (* ?cf1 ?cf2)))
      ...
      then (bind ?rv (+ (+ ?cf1 ?cf2) (* ?cf1 ?cf2)))
      else (bind ?rv (/ (+ ?cf1 ?cf2) (- 1 (min (abs ?cf1) (abs ?cf2))))))
  )
  ?rv
)
...
(defrule combinar-factores
  ...
  ?f2 <- (factor-certeza (propiedad saludable) (receta ?r) (valor ?cf2))
  (test (neq ?cf1 ?cf2))
  (test (or (neq ?cf1 0.0) (neq ?cf2 0.0))) ; evita combinar 0 + 0
  =>
  (retract ?f1 ?f2)
  (assert (factor-certeza (propiedad saludable) (receta ?r) 
...
(defrule inicializar-factor-certeza-saludable
  (modulo deducir-propiedades)
  (receta (nombre ?r))
  (not (factor-certeza (propiedad saludable) (receta ?r)))
  =>
  (assert (factor-certeza (propiedad saludable) (receta ?r) (valor 0.0)))
)
